...
class Manifest
{
    ...
    public static function downloadMultipleManifests($manifests, $mode = 'I', $name = 'Omniva manifest') {
      ...
      foreach($manifests as $manifest)
      {
        ...
        if(empty($manifest->orders))
          ...
          $pdf->SetFont('freeserif', '', 14);
          $shop_address = $manifest->sender->getAddress();
          $shop_addr = '<table cellspacing="0" cellpadding="1" border="0"><tr><td>' . date('Y-m-d H:i:s') . '</td><td>' . 'Sender address' . ':<br/>' . $manifest->sender->getPersonName() . '<br/>' . $shop_address->getStreet() . ', ' . $shop_address->getPostCode() . '<br/>' . $shop_address->getDeliveryPoint() . ', ' . $shop_address->getCountry() . '<br/></td></tr></table>';
