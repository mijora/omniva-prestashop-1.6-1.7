...
class OmnivaltShipping extends CarrierModule
{
    ...
    */
    ...
    INNER JOIN (
        -- Renkame naujausią datą kiekvienam id_order
        ...
        WHERE manifest IS NOT NULL 
          ...
          AND manifest != -1
        GROUP BY id_order
    ) latest_orders ON ooh.id_order = latest_orders.id_order 
                   AND ooh.date_add = latest_orders.max_date_add
        ...
        if ( $data_type == 'carriers' ) {
            ...
            foreach ( $methods_keys as $method_name => $method_keys ) {
                ...
                foreach ( $method_keys as $method_key ) {
                    $carriers_ref_ids[] = Configuration::get($method_key . '_reference');
                }
